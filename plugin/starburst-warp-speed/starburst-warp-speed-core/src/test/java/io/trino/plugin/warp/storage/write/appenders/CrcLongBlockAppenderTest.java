        ...
        extends BlockAppenderTest
{
    ...
    static Stream<Arguments> params()
    {
        ...
        long expectedShortDecimalMax = shortDecimalValues.stream().mapToLong(value -> Decimals.encodeShortScaledValue(value, shortDecimalType.getScale())).max().getAsLong();
        long expectedShortDecimalMin = shortDecimalValues.stream().mapToLong(value -> Decimals.encodeShortScaledValue(value, shortDecimalType.getScale())).min().getAsLong();
    ...
    private static BlockBuilder createShortDecimalBlockBuilder(DecimalType shortDecimalType, List<BigDecimal> values)
    {
        BlockBuilder shortDecimalBlockBuilder = shortDecimalType.createBlockBuilder(null, 5);

        for (BigDecimal value : values) {
            shortDecimalType.writeLong(shortDecimalBlockBuilder, Decimals.encodeShortScaledValue(value, shortDecimalType.getScale()));
        }
