...
/**
 * Adapted from {@code io.trino.operator.scalar.JsonExtract} to avoid dependency on trino-main.
 * <p>
...
public final class JsonExtract
{
    ...
    // Using JsonFactory instead of JsonMapper since we only need parser/generator, not ObjectMapper features.
    ...
    // Inlined from JsonUtil.createJsonParser (trino-main)
    ...
    {
        Integer parsed = Ints.tryParse(fieldName);
