            ...
            SetMultimap<WarpColumn, WarmupProperties> requiredWarmUpTypeMap)
    {
        ...
        for (int connectorBlockIdx = 0; connectorBlockIdx < columnsToWarm.size(); connectorBlockIdx++) {
            ...
            List<TransformedColumn> transformedColumns = requiredWarmUpTypeMap
                    .keySet()
                    .stream()
                    .filter(x -> x instanceof TransformedColumn transformedColumn && transformedColumn.getName().equalsIgnoreCase(warpColumn.getName()))
                    .map(x -> (TransformedColumn) x)
                    .toList();
            if (transformedColumns.isEmpty()) {
                warmUpElements = proxiedWarmupElements.get(warpColumn);
            }
            else {
        ...
        Comparator<Map.Entry<WarpColumn, Double>> comp = Comparator.comparingDouble(Map.Entry::getValue);
