        ...
        implements Provider<Procedure>
{
    ...
    private void doSyncPartitionMetadata(ConnectorSession session, ConnectorAccessControl accessControl, String schemaName, String tableName, String mode, boolean caseSensitive)
    {
        ...
        try {
            ...
            List<String> partitionsNamesInMetastore = metastore.getPartitionNames(identity, schemaName, tableName)
                    .orElseThrow(() -> new TableNotFoundException(schemaTableName));
            List<String> partitionsInMetastore = getPartitionsInMetastore(identity, schemaTableName, tableLocation, partitionsNamesInMetastore, metastore);
    ...
    private List<String> getPartitionsInMetastore(HiveIdentity identity, SchemaTableName schemaTableName, Path tableLocation, List<String> partitionsNames, SemiTransactionalHiveMetastore metastore)
    {
        ...
        for (List<String> partitionsNamesBatch : Lists.partition(partitionsNames, BATCH_GET_PARTITIONS_BY_NAMES_MAX_PAGE_SIZE)) {
            metastore.getPartitionsByNames(identity, schemaTableName.getSchemaName(), schemaTableName.getTableName(), partitionsNamesBatch).values().stream()
