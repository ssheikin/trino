...
public class TestCachingHiveMetastore
{
    ...
    /**
     * Test {@link CachingHiveMetastore#getPartition(HiveIdentity, Table, List)} followed by
     * {@link CachingHiveMetastore#getPartitionsByNames(HiveIdentity, Table, List)}.
     * <p>
    ...
    {
        Table table = metastore.getTable(IDENTITY, TEST_DATABASE, TEST_TABLE).orElseThrow();
        Optional<Partition> firstRead = metastore.getPartition(IDENTITY, table, List.of(TEST_PARTITION1_VALUE));
        assertThat(firstRead).isPresent();
        assertThat(firstRead.get().getValues()).isEqualTo(List.of(TEST_PARTITION1_VALUE));

        Map<String, Optional<Partition>> byName = metastore.getPartitionsByNames(IDENTITY, table, List.of(TEST_PARTITION1));
            ...
            throws Exception
    {
        ...
        try {
            ...
            Future<Map<String, Optional<Partition>>> read = executor.submit(() -> {
                barrier.await(10, SECONDS);
                return metastore.getPartitionsByNames(IDENTITY, table, List.of(TEST_PARTITION1));
            });
