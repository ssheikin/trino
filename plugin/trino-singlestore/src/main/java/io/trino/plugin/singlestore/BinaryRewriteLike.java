...
public class BinaryRewriteLike
        implements ConnectorExpressionRule<Call, ParameterizedExpression>
{
    ...
    private static final Pattern<Call> PATTERN = call()
            ...
            .with(argument(0).matching(variable().capturedAs(LIKE_VALUE).with(type().matching(VarcharType.class::isInstance))))
            .with(argument(1).matching(constant().capturedAs(LIKE_PATTERN).with(type().matching(VarcharType.class::isInstance))));
    ...
    public Optional<ParameterizedExpression> rewrite(Call expression, Captures captures, RewriteContext<ParameterizedExpression> context)
    {
        ...
        return Optional.of(new ParameterizedExpression(format("BINARY %s LIKE %s", value.get().expression(), pattern.get().expression()), parameters.build()));
    }
}
