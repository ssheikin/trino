...
import org.apache.trino.kudu.client.internals.SecurityManagerCompatibility;
import org.apache.trino.kudu.util.SecurityUtil;
        ...
        extends SimpleChannelInboundHandler<CallResponse>
{
    ...
    private static final ImmutableSet<RpcFeatureFlag> SUPPORTED_RPC_FEATURES =
            ImmutableSet.of(
    ...
    private AuthnTokenNotUsedReason authnTokenNotUsedReason;
     ...
     */
    private Set<RpcFeatureFlag> serverFeatures;
    ...
    {
        NegotiatePB.Builder builder = NegotiatePB.newBuilder()
                .setStep(NegotiateStep.NEGOTIATE);

        // Advertise our supported features
        // ----------------------------------
        for (RpcFeatureFlag flag : SUPPORTED_RPC_FEATURES) {
    ...
    private void sendSaslMessage(ChannelHandlerContext ctx, NegotiatePB msg)
    {
    ...
    @Override
    public void channelRead0(ChannelHandlerContext ctx, CallResponse msg)
            throws IOException
    {
        ...
        NegotiatePB response = parseSaslMsgResponse(msg);
    ...
    private NegotiatePB parseSaslMsgResponse(CallResponse response)
    {
        ...
        NegotiatePB.Builder saslBuilder = NegotiatePB.newBuilder();
    ...
    private void handleNegotiateResponse(ChannelHandlerContext ctx,
            ...
            throws IOException
    {
        checkState(response.getStep() == NegotiateStep.NEGOTIATE,
    ...
    private void chooseAndInitializeSaslMech(NegotiatePB response)
            throws KuduException
    {
        securityContext.refreshSubject();
        // Gather the set of server-supported mechanisms.
        Map<String, String> errorsByMech = new HashMap<>();
        Set<SaslMechanism> serverMechs = new HashSet<>();
        for (NegotiatePB.SaslMechanism mech : response.getSaslMechanismsList()) {
    ...
    {
        checkArgument(response.getAuthnTypesCount() <= 1,
    ...
    {
        ImmutableSet.Builder<RpcFeatureFlag> features = ImmutableSet.builder();
        for (RpcFeatureFlag feature : response.getSupportedFeaturesList()) {
    ...
    private void handleTlsMessage(ChannelHandlerContext ctx, NegotiatePB response)
            ...
            throws IOException
    {
        checkState(response.getStep() == NegotiateStep.TLS_HANDSHAKE);
        checkArgument(!response.getTlsHandshake().isEmpty(),
    ...
    private boolean sendPendingOutboundTls(ChannelHandlerContext ctx)
    {
        ...
        List<ByteString> bufs = new ArrayList<>();
    ...
    private void sendTokenExchange(ChannelHandlerContext ctx)
    {
        ...
        requireNonNull(authnToken);
        requireNonNull(sslHandshakeFuture);
        checkState(sslHandshakeFuture.isSuccess());
    ...
    {
        checkArgument(response.getStep() == NegotiateStep.TOKEN_EXCHANGE,
    ...
    private void handleJwtExchangeResponse(ChannelHandlerContext ctx, NegotiatePB response)
            ...
            throws SaslException
    {
        checkArgument(response.getStep() == NegotiateStep.JWT_EXCHANGE,
            ...
            throws SaslException, NonRecoverableException
    {
        ...
        builder.setStep(NegotiateStep.SASL_INITIATE);
        builder.addSaslMechanismsBuilder().setMechanism(chosenMech.name());
        state = State.AWAIT_SASL;
        sendSaslMessage(ctx, builder.build());
    }

    private void handleChallengeResponse(ChannelHandlerContext ctx, NegotiatePB response)
            ...
            throws SaslException, NonRecoverableException
    {
        ...
        builder.setStep(NegotiateStep.SASL_RESPONSE);
        sendSaslMessage(ctx, builder.build());
    }
    ...
    private void handleSuccessResponse(ChannelHandlerContext ctx, NegotiatePB response)
            ...
            throws IOException
    {
        checkState(saslClient.isComplete(),
     ...
     * the actual channel above the sharing validation runs and
     * throws an exception.
     * p
