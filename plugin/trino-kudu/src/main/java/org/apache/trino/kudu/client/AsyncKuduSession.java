...
import org.apache.trino.kudu.client.AsyncKuduClient.LookupType;
import org.apache.trino.kudu.util.AsyncUtil;
import org.apache.trino.kudu.util.LogThrottler;
import org.apache.trino.kudu.util.Slice;
 ...
 *
 * With {@link FlushMode#AUTO_FLUSH_SYNC AUTO_FLUSH_SYNC}, the timeout is set
 * on each {@linkplain #apply apply}()'d operation.
 * With {@link FlushMode#AUTO_FLUSH_BACKGROUND AUTO_FLUSH_BACKGROUND} and
 * {@link FlushMode#MANUAL_FLUSH MANUAL_FLUSH}, the timeout is assigned to a
 ...
 *
 * <ol>
 * <li>When using {@link FlushMode#MANUAL_FLUSH MANUAL_FLUSH} mode,
 * wait for one {@link #flush flush()} to {@code join()} before triggering another flush.
 * <li>When using {@link FlushMode#AUTO_FLUSH_SYNC AUTO_FLUSH_SYNC}
 * mode, wait for each {@link #apply apply()} to {@code join()} before applying another operation.
 * <li>Consider not using
 * {@link FlushMode#AUTO_FLUSH_BACKGROUND AUTO_FLUSH_BACKGROUND} mode.
        ...
        implements SessionConfiguration
{
     ...
     * buffers are still flushing, then the {@code apply} call
     * throws {@link PleaseThrottleException}.
     */
    ...
    private boolean ignoreAllDuplicateRows;
    private boolean ignoreAllNotFoundRows;
    ...
    {
        checkState(txnId > AsyncKuduClient.INVALID_TXN_ID);
        this.client = client;
        this.txnId = txnId;
        flushMode = FlushMode.AUTO_FLUSH_SYNC;
        consistencyMode = CLIENT_PROPAGATED;
    ...
    private static class ConvertBatchToListOfResponsesCB
            implements Callback<List<OperationResponse>,
            List<BatchResponse>>
    {
        ...
        @Override
        public List<OperationResponse> call(List<BatchResponse> batchResponses)
                throws Exception
        {
            ...
            for (BatchResponse batchResponse : batchResponses) {
                ...
                for (int i = 0; i < indexList.size(); i++) {
                    ...
                    checkState(responses[index] == null);
     ...
     * {@link FlushMode FlushMode}. Regardless
     ...
     * @see FlushMode FlushMode
     */
    public Deferred<OperationResponse> apply(final Operation operation)
            throws KuduException
    {
        requireNonNull(operation, "Cannot apply a null operation");
        checkArgument(operation.getTable().getAsyncClient() == client,
    ...
    private final class Buffer
    {
        ...
        private boolean flushNotificationFired;
    ...
    private static boolean injectLatencyBufferFlushCb;
