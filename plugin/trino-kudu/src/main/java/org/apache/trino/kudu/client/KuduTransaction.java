 ...
 * p
 * The {@link KuduTransaction}
 * implements {@link AutoCloseable} and should be
        ...
        implements AutoCloseable
{
    ...
    public AsyncKuduSession newAsyncKuduSession()
    {
        ...
        synchronized (isCommitStartedSync) {
            checkState(!isCommitStarted, "commit already started");
     ...
     */
    public byte[] serialize(SerializationOptions options)
            throws IOException
    {
    ...
    public static KuduTransaction deserialize(
            byte[] buf, AsyncKuduClient client)
            throws IOException
    {
    ...
    private CommitTransactionRequest doCommitTransaction()
            throws KuduException
    {
    ...
    private void commitWithMode(CommitMode mode)
            throws KuduException
    {
        synchronized (isInFlightSync) {
            checkState(isInFlight, ERRMSG_TXN_NOT_OPEN);
     ...
     */
    private Callback<Deferred<GetTransactionStateResponse>, GetTransactionStateResponse> isTransactionCommittedCb(final KuduRpc<GetTransactionStateResponse> rpc)
    {
        return resp -> {
            if (resp.hasCommitTimestamp()) {
                client.updateLastPropagatedTimestamp(resp.getCommitTimestamp());
    ...
    {
        checkState(keepaliveEnabled);
        checkArgument(txnId > AsyncKuduClient.INVALID_TXN_ID);
        synchronized (keepaliveTaskHandleSync) {
            checkState(keepaliveTaskHandle == null,
    ...
    private Callback<Void, Exception> getSendKeepTransactionAliveEB()
    {
        return e -> {
            ...
            else if (e instanceof NonRecoverableException ex) {
            ...
            if (scheduleNextRun) {
                checkArgument(nextRunAfterMillis >= 0);
                synchronized (keepaliveTaskHandleSync) {
                    if (!keepaliveTaskHandle.isCancelled()) {
                        keepaliveTaskHandle = delayedSendKeepTransactionAlive(
                                nextRunAfterMillis,
