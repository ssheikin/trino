...
import org.apache.kudu.shaded.io.netty.buffer.PooledByteBufAllocator;
import org.apache.kudu.shaded.io.netty.channel.ChannelOption;
import org.apache.kudu.shaded.io.netty.channel.EventLoopGroup;
import org.apache.kudu.shaded.io.netty.channel.nio.NioEventLoopGroup;
import org.apache.kudu.shaded.io.netty.channel.socket.nio.NioSocketChannel;
import org.apache.kudu.shaded.io.netty.util.HashedWheelTimer;
...
import org.apache.trino.kudu.util.AsyncUtil;
import org.apache.trino.kudu.util.NetUtil;
...
import static org.apache.trino.kudu.client.ExternalConsistencyMode.CLIENT_PROPAGATED;
        ...
        implements AutoCloseable
{
                            ...
                            Callback<Void, Exception> eb)
    {
                ...
                implements Callback<Void, ConnectToClusterResponse>
        {
             ...
             */
            @Override
            public Void call(ConnectToClusterResponse resp)
                    throws Exception
            {
        ...
        ConnectToCluster.run(masterTable, masterAddresses, null, defaultAdminOperationTimeoutMs,
                Connection.CredentialsPolicy.PRIMARY_CREDENTIALS).addCallbacks(new ReconnectToClusterCB(cb), eb);
    }
    ...
    private Deferred<KuduTable> getTableSchema(
            ...
            @Nullable KuduRpc<?> parent)
    {
        checkArgument(tableId != null || tableName != null);
            ...
            final KuduRpc<byte[]> fakeRpc)
    {
        ...
        getMasterTableLocationsPB(null)
                ...
                .addCallback(ignored -> {
                    ...
                    checkState(hasConnectedToMaster);
                    doExportAuthenticationCredentials(fakeRpc);
                    return null;
                })
     ...
     */
    @InterfaceStability.Unstable
    public void trustedCertificates(List<ByteString> certificates)
    ...
    {
        RemoteTablet tablet = requireNonNull(scanner.currentTablet());
            ...
            extends KuduRpc<R>
    {
        ...
        @Override
        Pair<R, Object> deserialize(CallResponse callResponse, String tsUUID)
     ...
     */
    private Deferred<GetTableLocationsResponsePB> locateTablet(KuduTable table,
                                                               byte[] partitionKey,
                                                               int fetchBatchSize,
                                                               KuduRpc<?> parentRpc)
    {
        ...
        Deferred<GetTableLocationsResponsePB> d;
     ...
     */
    Deferred<GetTableLocationsResponsePB> getMasterTableLocationsPB(KuduRpc<?> parentRpc)
    {
        // TODO(todd): stop using this 'masterTable' hack.
        return ConnectToCluster.run(masterTable, masterAddresses, parentRpc,
                defaultAdminOperationTimeoutMs, Connection.CredentialsPolicy.ANY_CREDENTIALS).addCallback(resp -> {
                                                          ...
                                                          final TimeoutTracker timeoutTracker)
    {
        ...
        checkArgument(startPartitionKey == null || startPartitionKey.length > 0,
                "use null for unbounded start partition key");
        checkArgument(endPartitionKey == null || endPartitionKey.length > 0,
    ...
    Deferred<List<LocatedTablet>> locateTable(final KuduTable table,
                                              ...
                                              long deadline)
    {
        final List<LocatedTablet> ret = new ArrayList<>();
                                               ...
                                               long deadline)
    {
        ...
        Callback<Deferred<List<KeyRange>>, List<LocatedTablet>> locateTabletCB = tablets -> {
            ...
            return Deferred.groupInOrder(deferreds).addCallbackDeferring(rangeLists -> {
                final List<KeyRange> ret = new ArrayList<>();
        ...
        final List<LocatedTablet> tablets = new ArrayList<>();
        return loopLocateTable(table,
                startPartitionKey,
                endPartitionKey,
                fetchBatchSize,
    ...
    /**
     * Translate master-provided information {@link TSInfoPB} on a tablet server into internal
     ...
     */
    private ServerInfo resolveTS(TSInfoPB tsInfoPB)
    ...
    public Deferred<ArrayList<Void>> shutdown()
    {
        ...
        final class DisconnectCB
                implements Callback<Deferred<ArrayList<Void>>,
    ...
    private static final class PingSupportsFeatureCallback
    ...
    public static final class AsyncKuduClientBuilder
    {
        ...
        {
            this.masterAddresses = new ArrayList<>(masterAddresses.size());
        ...
        {
            checkArgument(workerCount > 0, "workerCount should be greater than 0");
