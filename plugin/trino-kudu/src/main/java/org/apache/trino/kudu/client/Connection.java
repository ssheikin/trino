        ...
        extends SimpleChannelInboundHandler<Object>
{
     ...
     */
    @GuardedBy("lock")
    private ArrayList<QueuedMessage> queuedMessages = new ArrayList<>();

    /**
     * The result of the successful connection negotiation.
     */
    @GuardedBy("lock")
    private Negotiator.Success negotiationResult;

    /**
     * The result of failed connection negotiation.
     */
    @GuardedBy("lock")
    private Negotiator.Failure negotiationFailure;
    ...
    @Override
    @SuppressWarnings("FutureReturnValueIgnored")
    public void channelRead0(ChannelHandlerContext ctx, Object m)
            throws Exception
    {
        ...
        if (m instanceof Negotiator.Success) {
            ...
            try {
                ...
                checkState(state == State.NEGOTIATING);
            ...
            throws Exception
    {
        ...
        if (e instanceof KuduException) {
            ...
            checkState(!explicitlyDisconnected);
    ...
    private void cleanup(KuduException error)
    {
        ...
        try {
            if (state == State.TERMINATED) {
                ...
                checkState(queuedMessages == null);
                checkState(inflightMessages == null);
                return;
            }
            if (state == State.NEGOTIATION_FAILED) {
                checkState(negotiationFailure != null);
                checkState(inflightMessages.isEmpty());
