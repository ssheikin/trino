...
import org.apache.trino.kudu.util.StringUtil;
...
public class PartialRow
{
    ...
    PartialRow(PartialRow row)
    {
        ...
        this.varLengthData = new ArrayList<>(row.varLengthData.size());
    ...
    {
        ColumnSchema col = schema.getColumnByIndex(idx);
        checkState(columnsBitSet.get(idx), "Column %s is not set", col.getName());
    ...
    void setRaw(int index, byte[] value)
    {
        ...
        switch (type) {
            ...
            case DOUBLE:
            case DECIMAL: {
                checkArgument(value.length == column.getTypeSize());
    ...
    private static boolean isCellEqual(PartialRow a, PartialRow b, int index)
    {
        ...
        checkArgument(a.getSchema().equals(b.getSchema()));
        checkArgument(a.getSchema().getColumnByIndex(index).isKey());
        checkArgument(a.isSet(index));
        checkArgument(b.isSet(index));
    ...
    private static boolean isCellIncremented(PartialRow lower, PartialRow upper, int index)
    {
        // These checks are perhaps overly restrictive, but right now we only use
        // this method for checking fully-set keys.
        checkArgument(lower.getSchema().equals(upper.getSchema()));
        checkArgument(lower.getSchema().getColumnByIndex(index).isKey());
        checkArgument(lower.isSet(index));
        checkArgument(upper.isSet(index));
