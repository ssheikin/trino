...
import org.apache.trino.kudu.util.DateUtil;
import org.apache.trino.kudu.util.DecimalUtil;
import org.apache.trino.kudu.util.TimestampUtil;
...
public class KuduPredicate
{
                                                       ...
                                                       long value)
    {
        ...
        checkArgument(value <= maxValue && value >= minValue,
                                                       ...
                                                       BigDecimal value)
    {
        ...
        checkArgument(value.compareTo(maxValue) <= 0 && value.compareTo(minValue) >= 0,
    ...
    {
        checkArgument(column.equals(other.column),
        ...
        switch (type) {
            ...
            case EQUALITY: {
                if (other.type == PredicateType.EQUALITY) {
                    ...
                    checkState(other.type == PredicateType.IN_LIST);
            ...
            case RANGE: {
                if (other.type == PredicateType.EQUALITY || other.type == PredicateType.IN_LIST) {
                    ...
                    checkState(other.type == PredicateType.RANGE);
    ...
    private static KuduPredicate buildInList(ColumnSchema column, Collection<byte[]> values)
    {
        ...
        switch (values.size()) {
            ...
            default:
                return new KuduPredicate(column, values.toArray(new byte[values.size()][]));
        }
    }
    ...
    public static byte[] serialize(List<KuduPredicate> predicates)
            throws IOException
    {
    ...
    @InterfaceAudience.LimitedPrivate("kudu-mapreduce")
    public static List<KuduPredicate> deserialize(Schema schema, byte[] bytes)
            throws IOException
    {
    ...
    public String toString()
    {
        switch (type) {
            ...
            case IS_NOT_NULL:
                return String.format("`%s` IS NOT NULL", column.getName());
            case IS_NULL:
                return String.format("`%s` IS NULL", column.getName());
            case NONE:
                return String.format("`%s` NONE", column.getName());
            default:
    ...
    public int hashCode()
    {
        return Objects.hash(type, column, Arrays.hashCode(lower),
