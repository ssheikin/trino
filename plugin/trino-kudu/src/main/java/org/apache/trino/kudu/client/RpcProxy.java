...
import static org.apache.trino.kudu.client.KuduMetrics.RPC_REQUESTS_METRIC;
import static org.apache.trino.kudu.client.KuduMetrics.RPC_RESPONSE_METRIC;
import static org.apache.trino.kudu.client.KuduMetrics.RPC_RETRIES_METRIC;
import static org.apache.trino.kudu.client.KuduMetrics.counter;
...
class RpcProxy
{
                            ...
                            final KuduRpc<R> rpc)
    {
        ...
        try {
            ...
            if (staticNumFail > 0) {
                ...
                throw new RuntimeException("Forcing a failure on sendRpc");
            }
            if (!rpc.getRequiredFeatures().isEmpty()) {
                ...
                if (features != null &&
                        !features.contains(RpcFeatureFlag.APPLICATION_FEATURE_FLAGS)) {
            ...
            connection.enqueueMessage(rpcToMessage(client, rpc),
                    ...
                    {
                        @Override
                        public Void call(Connection.CallResponseInfo callResponseInfo)
                                throws Exception
                        {
                            try {
                                             ...
                                             KuduException ex)
    {
        ...
        try {
            if (decoded != null) {
                checkState(!(decoded.getFirst() instanceof Exception));
