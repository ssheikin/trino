        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testVerifyAutoGeneratedSchemas()
    {
        ...
        assertThat(Stream.of(requireNonNull(new File(actualSchemasDirectory).listFiles(), "schema files do not exist"))
                .filter(file -> !file.isDirectory())
                .map(File::getName)
                .toList())
                .isEmpty();
        ...
        for (TpchTable<?> table : TPCH_TABLES) {
            ...
            File actualAutoGeneratedRsdFile = Paths.get(actualSchemasDirectory, tableName + ".rsd").toFile();
            File expectedAutoGeneratedRsdFile1 = Paths.get(EXPECTED_AUTO_GENERATED_SCHEMAS_DIRECTORY, tableName + "1.rsd").toFile();
            File expectedAutoGeneratedRsdFile2 = Paths.get(EXPECTED_AUTO_GENERATED_SCHEMAS_DIRECTORY, tableName + "2.rsd").toFile();

            if (table.getTableName().equals(REGION.getTableName())) {
                File expectedAutoGeneratedRsdFile3 = Paths.get(EXPECTED_AUTO_GENERATED_SCHEMAS_DIRECTORY, tableName + "3.rsd").toFile();
