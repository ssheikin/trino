...
/**
 * End-to-end MV substitution test with Elasticsearch as the source. The Elasticsearch connector is
 * read-only, so unlike the JDBC-backed sources the base tables cannot be created, populated or
 * dropped through Trino (or a backend SQL executor); they are seeded directly as Elasticsearch
...
public class TestIcebergMvOnElasticsearchSourceSubstitution
        extends AbstractIcebergMvSubstitutionTest
{
    ...
    protected QueryRunner createSourceQueryRunner(Session defaultSession, CatalogSchemaName sourceSchema)
            throws Exception
    {
        ElasticsearchServer server = closeAfterClass(new ElasticsearchServer(ELASTICSEARCH_7_IMAGE));
    ...
    {
        // "tpch" is the Elasticsearch connector's default schema name (see ElasticsearchQueryRunner).
        return new CatalogSchemaName("elasticsearch", "tpch");
    }

    // ElasticsearchQueryRunner already provisions the tpch catalog, and the Elasticsearch schema is
    ...
    {
        // Elasticsearch exposes a nested object as a Trino ROW, so the sub-field is a struct
    ...
    /**
     * The read-only Elasticsearch source cannot execute SQL. The only source statement the inherited
    ...
    protected SqlExecutor sourceSqlExecutor()
    {
        return sql -> {
            ...
            checkArgument(
                    ...
                    "Read-only Elasticsearch source supports only DROP TABLE, got: %s",
                    sql);
