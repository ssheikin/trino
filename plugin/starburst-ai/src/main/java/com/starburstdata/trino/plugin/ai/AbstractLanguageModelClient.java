        ...
        implements LanguageModelClient
{
    ...
    protected AbstractLanguageModelClient(String model, PromptProvider promptProvider)
    {
        this.generateModel = requireNonNull(model, "model is null");
        this.promptProvider = requireNonNull(promptProvider, "promptProvider is null");
        this.topLevelSystemPrompts = promptProvider.system();
        this.analyzeSentimentModelAndPrompt = create("analyzeSentiment", generateModel, topLevelSystemPrompts, promptProvider.analyzeSentiment(), promptProvider.analyzeSentimentSystem());
        this.classifyModelAndPrompt = create("classify", generateModel, topLevelSystemPrompts, promptProvider.classify(), promptProvider.classifySystem());
        this.fixGrammarModelAndPrompt = create("fixGrammar", generateModel, topLevelSystemPrompts, promptProvider.fixGrammar(), promptProvider.fixGrammarSystem());
        this.maskModelAndPrompt = create("mask", generateModel, topLevelSystemPrompts, promptProvider.mask(), promptProvider.maskSystem());
        this.translateModelAndPrompt = create("translate", generateModel, topLevelSystemPrompts, promptProvider.translate(), promptProvider.translateSystem());
    }
