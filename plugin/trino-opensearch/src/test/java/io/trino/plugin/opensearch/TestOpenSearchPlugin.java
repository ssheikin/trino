...
final class TestOpenSearchPlugin
{
    @Test
    void testGetSecuritySensitivePropertyNames()
    {
        Plugin plugin = new OpenSearchPlugin();
        ...
        Map<String, String> config = ImmutableMap.of(
                ...
                "opensearch.auth.user", "user",
                "opensearch.host", "localhost");

        Set<String> sensitiveProperties = factory.getSecuritySensitivePropertyNames("catalog", config, new TestingConnectorContext());

        assertThat(sensitiveProperties).containsExactlyInAnyOrder("non-existent-property", "opensearch.auth.password");
    }
}
