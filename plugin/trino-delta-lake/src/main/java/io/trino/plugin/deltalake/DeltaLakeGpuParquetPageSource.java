...
public class DeltaLakeGpuParquetPageSource
        implements ConnectorGpuPageSource
{
    ...
    record GpuParquetFileColumn(DeltaLakeColumnHandle column, String parquetName, int parquetIndex)
    ...
    record GpuConstantColumn(DeltaLakeColumnHandle column, Type type, @Nullable Object value)
            implements GpuOutputColumn
    {
        GpuConstantColumn
        {
    ...
    public DeltaLakeGpuParquetPageSource(
    ...
    private @Move GpuPage convertToGpuPage(@Borrow Table table)
    {
        if (table.getNumberOfColumns() != parquetColumnNames.length) {
            throw new TrinoException(DELTA_LAKE_BAD_DATA, format("Expected %d columns from cuDF but got %d", parquetColumnNames.length, table.getNumberOfColumns()));
        }
            ...
            for (int i = 0; i < outputColumns.size(); i++) {
                columns[i] = switch (outputColumns.get(i)) {
                    case GpuParquetFileColumn parquetColumn -> {
                        ...
                        Type trinoType = parquetColumn.column().type();
                        DType expectedDType = toDType(trinoType)
                                .orElseThrow(() -> new UnsupportedOperationException("Unsupported type: " + trinoType));
                        @Own ColumnVector evolved = evolveColumn(parquetColumn.column().baseColumnName(), cudfColumn, expectedDType, trinoType);
