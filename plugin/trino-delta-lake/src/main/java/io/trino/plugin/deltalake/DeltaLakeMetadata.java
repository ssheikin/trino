        ...
        implements ConnectorMetadata
{
    ...
    public TableSnapshot getSnapshot(ConnectorSession session, TransactionLogReader transactionLogReader, SchemaTableName table, String tableLocation, Optional<Long> atVersion)
    {
        ...
        try {
            TableSnapshot snapshot = transactionLogAccess.loadSnapshot(session, transactionLogReader, table, tableLocation, version);
            ...
            Optional<ConnectorTableVersion> endVersion)
    {
        ...
        TableSnapshot tableSnapshot = getSnapshot(session, transactionLogReaderFactory.createReader(table), tableName, tableLocation, endVersion.map(version -> getVersion(session, fileSystem, tableLocation, version, metadataFetchingExecutor)));
    ...
    private RelationCommentMetadata getRelationCommentMetadata(ConnectorSession session, TrinoFileSystem fileSystem, SchemaTableName tableName)
    {
        ...
        try {
            ...
            TransactionLogReader transactionLogReader = transactionLogReaderFactory.createReader(convertToDeltaMetastoreTable(table));
            TableSnapshot snapshot = getSnapshot(session, transactionLogReader, tableName, tableLocation, Optional.empty());
        ...
        return tables.stream()
                .flatMap(tableName -> {
                    try {
                        ...
                        TransactionLogReader transactionLogReader = transactionLogReaderFactory.createReader(convertToDeltaMetastoreTable(table));
                        TableSnapshot snapshot = transactionLogAccess.loadSnapshot(session, transactionLogReader, tableName, tableLocation, Optional.empty());
                        MetadataEntry metadata = transactionLogAccess.getMetadataEntry(session, snapshot);
            ...
            if (!replaceExistingTable && transactionLogFileExists) {
                ...
                if (replaceExistingTable) {
                    long lastKnownBackfilledVersion = isCatalogOwnedTable(tableHandle.getProtocolEntry())
                            ...
                            : commitVersion;
                    writeCheckpointIfNeeded(session, schemaTableName, location, tableHandle.getReadVersion(), checkpointInterval, lastKnownBackfilledVersion);
                }
            }
        }
        catch (IOException e) {
            ...
            Collection<ComputedStatistics> computedStatistics)
    {
        ...
        try {
            ...
            if (handle.readVersion().isEmpty()) {
                ...
                transactionLogWriter = transactionLogWriterFactory.createFileSystemWriter(session, location);
            }
            appendTableEntries(
                    commitVersion,
                    transactionLogWriter,
            ...
            if (handle.readVersion().isPresent()) {
                ...
                try (Stream<AddFileEntry> activeFiles = transactionLogAccess.getActiveFiles(
                        session,
                        transactionLogReaderFactory.createReader(deltaLakeTableHandle),
            ...
            if (handle.replace() && handle.readVersion().isPresent()) {
                long lastKnownBackfilledVersion = isCatalogOwnedTable(handle.protocolEntry())
                        ? getMandatoryCurrentVersion(fileSystemFactory.create(session), handle.location(), handle.readVersion().getAsLong())
                        : commitVersion;
                writeCheckpointIfNeeded(session, schemaTableName, handle.location(), handle.readVersion().getAsLong(), handle.checkpointInterval(), lastKnownBackfilledVersion);
            }
    ...
    public void addColumn(ConnectorSession session, ConnectorTableHandle tableHandle, ColumnMetadata newColumnMetadata, ColumnPosition position)
    {
        ...
        if (!newColumnMetadata.isNullable()) {
            ...
            try (Stream<AddFileEntry> addFileEntries = transactionLogAccess.getActiveFiles(
                    session,
                    transactionLogReaderFactory.createReader(handle),
            ...
            Collection<ComputedStatistics> computedStatistics)
    {
        ...
        try {
            ...
            writeCommitted = true;
            long lastKnownBackfilledVersion = isCatalogOwnedTable(handle.protocolEntry())
                    ? getMandatoryCurrentVersion(fileSystemFactory.create(session), handle.location(), handle.readVersion())
                    : commitVersion;
            writeCheckpointIfNeeded(session, handle.tableName(), handle.location(), handle.readVersion(), handle.metadataEntry().getCheckpointInterval(), lastKnownBackfilledVersion);
            ...
            throws IOException
    {
        ...
        TransactionLogWriter transactionLogWriter = transactionLogWriterFactory.createWriter(session, insertTableHandle.location(), insertTableHandle.metadataEntry(), insertTableHandle.protocolEntry());
            ...
            Collection<ComputedStatistics> computedStatistics)
    {
        ...
        try {
            ...
            long lastKnownBackfilledVersion = isCatalogOwnedTable(handle.getProtocolEntry())
                    ? getMandatoryCurrentVersion(fileSystemFactory.create(session), handle.location(), handle.getReadVersion())
                    : commitVersion;
            writeCheckpointIfNeeded(session, handle.getSchemaTableName(), handle.getLocation(), handle.getReadVersion(), checkpointInterval, lastKnownBackfilledVersion);
        }
    ...
    private void finishOptimize(ConnectorSession session, DeltaLakeTableExecuteHandle executeHandle, Collection<Slice> fragments, List<Object> splitSourceInfo)
    {
        ...
        try {
            ...
            long lastKnownBackfilledVersion = isCatalogOwnedTable(optimizeHandle.getProtocolEntry())
                    ? getMandatoryCurrentVersion(fileSystemFactory.create(session), tableLocation, optimizeHandle.getCurrentVersion().orElseThrow())
                    : commitVersion;
    ...
    private long commitOptimizeOperation(
            ...
            throws IOException
    {
        TransactionLogWriter transactionLogWriter = transactionLogWriterFactory.createWriter(session, tableLocation, optimizeHandle.getMetadataEntry(), optimizeHandle.getProtocolEntry());
    ...
    {
        return getSnapshot(session, transactionLogReaderFactory.createReader(table), table.getSchemaTableName(), table.getLocation(), Optional.of(table.getReadVersion()));
    }
            ...
            long newVersion)
    {
        try {
            ...
            TableSnapshot snapshot = getSnapshot(session, transactionLogReader, table, tableLocation, Optional.of(readVersion));
            ...
            // Now, the catalog owned table possible meet this condition
            ...
            TableSnapshot updatedSnapshot = snapshot.getUpdatedSnapshot(session, transactionLogReader, fileSystemFactory.create(session), Optional.of(newVersion)).orElseThrow();
    ...
    private Optional<SystemTable> getRawSystemTable(ConnectorSession session, SchemaTableName systemTableName)
    {
        ...
        TransactionLogReader transactionLogReader = transactionLogReaderFactory.createReader(table.get());
        ...
        return switch (tableType.get()) {
            ...
            case TRANSACTIONS -> Optional.of(new DeltaLakeTransactionsTable(
                    ...
                    typeManager,
                    transactionLogReader));
            case PROPERTIES -> Optional.of(new DeltaLakePropertiesTable(systemTableName, tableLocation, transactionLogAccess, transactionLogReader));
            case PARTITIONS -> Optional.of(new DeltaLakePartitionsTable(session, systemTableName, tableLocation, transactionLogAccess, typeManager, transactionLogReader));
        };
    }
    ...
    private OptionalLong executeDelete(ConnectorSession session, ConnectorTableHandle handle, String operation)
    {
        ...
        try {
            ...
            long lastKnownBackfilledVersion = isCatalogOwnedTable(tableHandle.getProtocolEntry())
                    ? getMandatoryCurrentVersion(fileSystemFactory.create(session), tableHandle.location(), tableHandle.getReadVersion())
