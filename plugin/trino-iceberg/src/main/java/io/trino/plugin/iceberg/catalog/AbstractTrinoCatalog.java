        ...
        implements TrinoCatalog
{
    ...
    protected Optional<String> createOrUpdateMaterializedViewRefreshJob(ConnectorSession session, SchemaTableName viewName, Optional<String> existingJobId, Optional<RefreshSchedule> newSchedule)
    {
        if (newSchedule.isPresent()) {
            ...
            if (existingJobId.isPresent() && workScheduler.updateJobSchedule(session, existingJobId.get(), newSchedule.get())) {
            ...
            existingJobId.ifPresent(jobId -> workScheduler.deleteJobSchedule(session, jobId));
            ...
            ExecutorService icebergScanExecutor)
    {
        ...
        if (isMaterializedViewStorage(storageTableName.getTableName())) {
            ...
            Map<String, String> viewProperties = createMaterializedViewProperties(session, Location.of(tableOperations.current().metadataFileLocation()), refreshJobId, isSubstitutionEnabled(materializedViewProperties), getIncrementalColumn(materializedViewProperties));
