        ...
        implements ConnectorPageSinkProvider
{
    ...
    public ConnectorPageSink createPageSink(ConnectorTransactionHandle transactionHandle, ConnectorSession session, ConnectorTableExecuteHandle tableExecuteHandle, Optional<ConnectorTableCredentials> tableCredentials, ConnectorPageSinkId pageSinkId)
    {
        ...
        return switch (executeHandle.procedureId()) {
            ...
            case GENERATE_EMBEDDINGS -> createGenerateEmbeddingsPageSink(session, executeHandle, tableCredentials.map(IcebergTableCredentials.class::cast).get());
            case OPTIMIZE_MANIFESTS, DROP_EXTENDED_STATS, ROLLBACK_TO_SNAPSHOT, EXPIRE_SNAPSHOTS, REMOVE_ORPHAN_FILES, ADD_FILES, ADD_FILES_FROM_TABLE, OPTIMIZE_POSITION_DELETES, REMOVE_DANGLING_DELETE_FILES, CREATE_CHANGELOG_VIEW -> {
    ...
    private ConnectorPageSink createGenerateEmbeddingsPageSink(ConnectorSession session, IcebergTableExecuteHandle executeHandle, IcebergTableCredentials tableCredentials)
    {
