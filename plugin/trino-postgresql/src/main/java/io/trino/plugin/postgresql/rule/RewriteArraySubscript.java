...
import static io.trino.plugin.postgresql.PostgreSqlSessionProperties.isEnableArraySubscriptPushdown;
...
public class RewriteArraySubscript
{
            ...
            implements ConnectorExpressionRule<Call, ParameterizedExpression>
    {
        ...
        {
            return isEnableArraySubscriptPushdown(session);
        }
        ...
        public Optional<ParameterizedExpression> rewrite(Call call, Captures captures, RewriteContext<ParameterizedExpression> context)
        {
            ...
            return Optional.of(rewriteArraySubscriptExpression(columnHandle.getColumnName(), index.get(), arrayElementTypeHandle));
        }
    }
            ...
            implements ProjectFunctionRule<JdbcExpression, ParameterizedExpression>
    {
        ...
        public Optional<JdbcExpression> rewrite(ConnectorExpression projectionExpression, Captures captures, RewriteContext<ParameterizedExpression> context)
        {
            ...
            ParameterizedExpression parameterizedExpression = rewriteArraySubscriptExpression(columnHandle.getColumnName(), index.get(), arrayElementTypeHandle);
    ...
    private static ParameterizedExpression rewriteArraySubscriptExpression(String columnName, ParameterizedExpression index, JdbcTypeHandle arrayElementTypeHandle)
    {
        return new ParameterizedExpression(
                "(CASE WHEN (%2$s > 0 AND ARRAY_NDIMS(%1$s) = 1 AND ARRAY_LENGTH(%1$s, 1) >= %2$s) THEN %1$s ELSE ARRAY[null, ARRAY[(SELECT null)]]::%3$s[] END)[%2$s]"
                        .formatted(columnName, index.expression(), arrayElementTypeHandle.jdbcTypeName().get()),
