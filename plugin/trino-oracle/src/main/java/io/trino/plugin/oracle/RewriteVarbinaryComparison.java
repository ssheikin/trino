...
public class RewriteVarbinaryComparison
        implements ConnectorExpressionRule<Call, ParameterizedExpression>
{
    ...
    private static final Pattern<Call> PATTERN = call()
            ...
            .with(argument(0).matching(variable().with(type().matching(type -> type instanceof VarbinaryType)).capturedAs(FIRST_ARGUMENT)))
            .with(argument(1).matching(variable().with(type().matching(type -> type instanceof VarbinaryType)).capturedAs(SECOND_ARGUMENT)));
    ...
    public Optional<ParameterizedExpression> rewrite(Call expression, Captures captures, RewriteContext<ParameterizedExpression> context)
    {
        ...
        if (!(supportsPushdown(firstArgument, context) && supportsPushdown(secondArgument, context))) {
    ...
    static boolean supportsPushdown(Variable variable, RewriteContext<?> context)
    {
        return ((JdbcColumnHandle) context.getAssignment(variable.getName())).getJdbcTypeHandle().jdbcType() == OracleTypes.VARBINARY;
    }
}
