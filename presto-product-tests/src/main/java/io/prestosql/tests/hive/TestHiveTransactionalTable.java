        ...
        extends HiveProductTest
{
    ...
    public void testSimpleUnpartitionedTransactionalInsert()
    {
        withTemporaryTable("unpartitioned_transactional_insert", true, false, NONE, tableName -> {
            ...
            String explainOutput = (String) onPresto().executeQuery("EXPLAIN " + insertQuery).row(0).get(0);
            Assertions.assertThat(explainOutput).contains("Output partitioning: hive:buckets=1");

            onPresto().executeQuery(insertQuery);
    ...
    public void testReadAfterMultiInsertAndDelete()
    {
        ...
        withTemporaryTable("partitioned_multi_insert", true, true, BucketingType.BUCKETED_V1, tableName -> {
            withTemporaryTable("tmp_data_table", false, false, NONE, dataTableName -> {
                onPresto().executeQuery(format("CREATE TABLE %s (a int, b int, c varchar(5)) WITH " +
                        "(transactional = true, partitioned_by = ARRAY['c'], bucketed_by = ARRAY['a'], bucket_count = 2)", tableName));
                onPresto().executeQuery(format("CREATE TABLE %s (x int)", dataTableName));
                onPresto().executeQuery(format("INSERT INTO %s VALUES 1", dataTableName));
        ...
        withTemporaryTable("test_insert_row_id_correctness", true, false, NONE, tableName -> {
            ...
            onPresto().executeQuery(format(
                    "CREATE TABLE %s\n"
                            ...
                            + "WITH NO DATA",
                    tableName));
            onPresto().executeQuery(format("INSERT INTO %s select * from tpch.tiny.supplier", tableName));
            ...
            assertThat(query("SELECT count(*)" + queryTarget))
                    ...
                    .containsOnly(row(1));
            assertThat(onPresto().executeQuery("DELETE" + queryTarget))
    ...
    {
        verifySelect("onPresto", onPresto(), "selecting on Presto", select, whereClause, rows);
        verifySelect("onHive", onHive(), "selecting on Hive", select, whereClause, rows);
    }

    private void verifySelect(String name, QueryExecutor executor, String description, String select, String whereClause, QueryAssert.Row... rows)
    {
