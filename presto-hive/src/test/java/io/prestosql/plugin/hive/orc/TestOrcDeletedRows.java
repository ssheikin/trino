...
public class TestOrcDeletedRows
{
    ...
    public void testDeleteLocations()
    {
        ...
        addDeleteDelta(deleteDeltaLocationsBuilder, 4L, 4L, OptionalInt.of(0), partitionDirectory);
        addDeleteDelta(deleteDeltaLocationsBuilder, 7L, 7L, OptionalInt.of(0), partitionDirectory);
    ...
    private void addDeleteDelta(DeleteDeltaLocations.Builder deleteDeltaLocationsBuilder, long minWriteId, long maxWriteId, OptionalInt statementId, Path path)
    {
        ...
        if (statementId.isPresent()) {
            ...
            deleteDeltaLocationsBuilder.addDeleteDelta(deleteDeltaPath, minWriteId, maxWriteId, OptionalInt.empty());
        }
    }
    ...
    {
        DeleteDeltaLocations.Builder deleteDeltaLocationsBuilder = DeleteDeltaLocations.builder(partitionDirectory);
        addDeleteDelta(deleteDeltaLocationsBuilder, 4L, 4L, OptionalInt.empty(), partitionDirectory);

        OrcDeletedRows deletedRows = createOrcDeletedRows(deleteDeltaLocationsBuilder.build().orElseThrow(() -> new RuntimeException("empty")));
        ...
        // page with no deleted rows
        testPage = createTestPage(10, 20);
        block = deletedRows.getMaskDeletedRowsFunction(testPage).apply(testPage.getBlock(1));
        assertEquals(block.getPositionCount(), 10);
    }
}
