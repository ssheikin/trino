...
class TestGpuSemiJoinMemory
{
    ...
    void testEmptyProbe(NullsProvider nullsProvider)
    {
        testProbe(BIGINT, 1, 0, nullsProvider);
    }

    @ParameterizedTest
    @EnumSource(NullsProvider.class)
    void testEmptyBuild(NullsProvider nullsProvider)
    {
        testProbe(BIGINT, 0, 1, nullsProvider);
    }

    @ParameterizedTest
    @EnumSource(NullsProvider.class)
    void testBigintKey(NullsProvider nullsProvider)
    {
        testProbe(BIGINT, 1, 1, nullsProvider);
    }

    @ParameterizedTest
    @EnumSource(NullsProvider.class)
    void testVarcharKey(NullsProvider nullsProvider)
    {
        testProbe(VARCHAR, 1, 1, nullsProvider);
    }

    @ParameterizedTest
    @EnumSource(NullsProvider.class)
    void testDecimalKey(NullsProvider nullsProvider)
    {
        testProbe(createDecimalType(18, 3), 1, 1, nullsProvider);
    }

    private void testProbe(Type keyType, int buildPageCount, int probePageCount, NullsProvider nullsProvider)
    {
        ...
        try (GpuDeviceMemoryUsageValidation memoryValidation = GpuDeviceMemoryUsageValidation.createAndRegister(context, 1024 * 1024)) {
            ...
            GpuSemiJoinBuild.Factory buildFactory = new GpuSemiJoinBuild.Factory(setSupplier, 0);
            ...
            try (var buildSource = new SettableGpuOperation();
                ...
                memoryValidation.withoutValidation(() -> {
                    Iterator<Page> buildPages = createPages(List.of(keyType), nullsProvider, buildPageCount, BUILD_ROWS, false);
                            ...
                            case Yielded() -> {
                                if (!buildSource.hasPending()) {
                                    if (buildPages.hasNext()) {
                                        GpuPage next = getOnlyElement(copyToDevice(List.of(buildPages.next()), List.of(keyType)));
                ...
                Iterator<Page> probePages = createPages(List.of(keyType), nullsProvider, probePageCount, PROBE_ROWS_PER_PAGE, true);
                GpuSemiJoin.Factory probeFactory = new GpuSemiJoin.Factory(setSupplier, 0);
                                ...
                                if (!probeSource.hasPending()) {
                                    memoryValidation.withoutValidation(() -> {
                                        if (probePages.hasNext()) {
                                            GpuPage next = getOnlyElement(copyToDevice(List.of(probePages.next()), List.of(keyType)));
