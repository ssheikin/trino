...
public class BenchmarkLongArrayBlockEncoding
{
    @Benchmark
    ...
    public void testBenchmarkData()
    {
        for (String dataCharacteristic : new String[] {"random", "delta_friendly", "rle_friendly"}) {
            ...
            for (Block inputBlock : data.blocks) {
                ...
                assertBlockEquals(BIGINT, inputBlock, outputBlock);
            }
        }
    }
    ...
    public static class BenchmarkData
    {
        ...
        public void setup()
        {
            ...
            encoding = new LongArrayAdaptiveBlockEncoding(true);

            blocks = new Block[BLOCK_COUNT];
            for (int i = 0; i < BLOCK_COUNT; i++) {
                BlockBuilder builder = BIGINT.createFixedSizeBlockBuilder(POSITIONS_PER_BLOCK);
                for (long value : prepareData()) {
        ...
        private long[] prepareData()
        {
            ...
            switch (dataCharacteristic) {
                case "random" -> {
                    return LongStream.range(0, POSITIONS_PER_BLOCK)
                            .map(_ -> iterator.next().extendedPriceInCents())
                            .toArray();
                }
                case "delta_friendly" -> {
                    long[] values = LongStream.range(0, POSITIONS_PER_BLOCK)
                            .map(_ -> iterator.next().extendedPriceInCents())
                            .toArray();
                    Arrays.sort(values);
                    return values;
    ...
    static void main()
            throws Exception
    {
        benchmark(BenchmarkLongArrayBlockEncoding.class).run();
    }
}
