...
public class TestLeastWastedEffortTaskLowMemoryKiller
{
    ...
    public void testWillNotKillTaskForQueryWithoutTaskRetriesEnabled()
    {
        ...
        assertThat(lowMemoryKiller.chooseTargetToKill(
                toRunningQueryInfoList(queries),
                toNodeMemoryInfoList(memoryPool, queries, tasks))).isEqualTo(Optional.empty());
    }
    ...
    private void testKillsBiggestTasksIfAllExecuteSameTime(Duration scheduledTime, Duration blockedTime)
    {
        ...
        assertThat(lowMemoryKiller.chooseTargetToKill(
                ...
                toNodeMemoryInfoList(memoryPool, queries, tasks))).isEqualTo(Optional.of(KillTarget.selectedTasks(
                ImmutableSet.of(
                        taskId("q_1", 1),
                        taskId("q_2", 6)))));
    }
