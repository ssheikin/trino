...
public record FieldMapping(Map<Integer, Integer> fieldIndexMapping)
{
    public static final FieldMapping EMPTY = new FieldMapping(ImmutableMap.of());
    ...
    {
        Map<Integer, Integer> inversedMapping = newHashMap(fieldIndexMapping.size());
        fieldIndexMapping.forEach((key, value) -> {
            checkState(isNull(inversedMapping.put(value, key)), "cannot inverse mapping");
        });

        return new FieldMapping(inversedMapping);
    }
