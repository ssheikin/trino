...
- [FieldReference](../dialect/trino/operation/FieldReference.java): selects a row field by index,
...
2. Operations can have a list of [Regions](Region.java) containing [Blocks](Block.java)
   to express nested logic. A Block is a lambda: it takes parameters and contains some code,
   ending with a terminal operation, like [Return](../dialect/trino/operation/Return.java).
   For example, [Filter](../dialect/trino/operation/Filter.java) has a block
   with the predicate, and [CorrelatedJoin](../dialect/trino/operation/CorrelatedJoin.java) has a block with the subquery program. The
   code in a block is a list of operations, which forms a recursive structure.
   Grouping Blocks in Regions allows to express more complex logic, as the Blocks within a
   region form a control flow graph.
