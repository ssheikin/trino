...
{
    public static final String NAME = "dynamic_filter_source";
    ...
    public static final Set<TrinoAttributeSignature<?>> OPERATION_ATTRIBUTES = ImmutableSet.of(DYNAMIC_FILTER_IDS);
    ...
    {
        checkArgument(arguments.size() == 1, "DynamicFilterSource operation must have exactly one argument: the input relation");
        checkArgument(regions.size() == 1, "DynamicFilterSource operation must have exactly one region: the dynamic filter target selector");
