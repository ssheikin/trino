...
public final class LongBloomFilter
{
    ...
    private LongBloomFilter(long[] bloom, int maxDistinctHashes)
    {
        this.bloom = bloom;
        this.bloomSizeMask = bloom.length - 1;
        this.maxDistinctHashes = maxDistinctHashes;
    }
    ...
    public static LongBloomFilter fromJson(
            ...
            @JsonProperty("maxDistinctHashes") int maxDistinctHashes)
    {
        return new LongBloomFilter(bloom, maxDistinctHashes);
    }
    ...
    @JsonProperty
    public int getMaxDistinctHashes()
    {
        return maxDistinctHashes;
    }
    ...
    public void merge(LongBloomFilter other)
    {
        ...
        maxDistinctHashes += other.getMaxDistinctHashes();
    ...
    {
        checkArgument(bloom.length == other.bloom.length, "Bloom filters must have the same size");
        maxDistinctHashes = Math.min(maxDistinctHashes, other.getMaxDistinctHashes());
        for (int i = 0; i < bloom.length; i++) {
            bloom[i] &= other.bloom[i];
        }
    }
