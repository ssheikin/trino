...
public class LetOperationMetadata
        implements TrinoOperationMetadata
{
    ...
    {
        checkArgument(arguments.size() == 1, "Let operation must have exactly one argument: the bound value");
        checkArgument(regions.size() == 1, "Let operation must have exactly one region: the body");
        ...
        return new Let(
                ...
                getOnlyElement(regions).getOnlyBlock().withLabel("^body"),
                Attributes.empty(),
                derivedAttributes);
    }
    ...
    {
        return LetOperationMetadata::deriveAttributes;
    }

    public static Attributes deriveAttributes(Attributes currentAttributes, List<Attributes> childAttributes)
    {
        checkArgument(childAttributes.size() == 2, "Let operation must have exactly two child attributes maps: one for the bound value, and one for the body");
