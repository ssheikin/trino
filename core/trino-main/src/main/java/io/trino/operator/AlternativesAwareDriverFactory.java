        ...
        implements SplitDriverFactory
{
    ...
    public Driver createDriver(DriverContext driverContext, Optional<ScheduledSplit> optionalSplit)
    {
        ...
        TableHandle chosen = alternativeChooser.chooseAlternative(session, split.split(), alternatives.keySet());

        AlternativeDriverFactory alternative = alternatives.get(chosen);
        return alternative.driverFactory().createDriver(driverContext.setAlternativeId(alternative.id()));
    }
