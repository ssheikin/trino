...
public class TestDemote
{
    ...
    public void testDemoteDefaultWarmup(TestFormat testFormat)
            throws IOException
    {
        execute(testFormat, TableType.warp, true);
    }

    @Test(groups = {WARP_SPEED_HIVE, PROFILE_SPECIFIC_TESTS}, dataProvider = "syntheticWarp")
    public void testDemotePriorityWarmup(TestFormat testFormat)
            throws IOException
    {
        execute(testFormat, TableType.warp, false);
    }
...
//                .filter(testFormat -> testFormat.name().equals("check_limit"))
                .map(testFormat -> updateTableType(testFormat, tableType))
                .iterator();
    }

    private void execute(TestFormat testFormat, TableType tableType, boolean defaultWarming)
            throws IOException
    {
        ...
        try (QueryExecutor queryExecutor = onTrino()) {
            ...
            double demoteThreshold = ((((double) currentUsage / totalCapacity) * 100) - 10) / 100;

            int ttlInSeconds = 1;

            // wait for TTL to expire
            Thread.sleep(Duration.ofSeconds(ttlInSeconds * 2).toMillis());
            ...
            if (defaultWarming) {
                assertThat(getValue(demoterStatsAfter, JMXCachingConstants.WarmupDemoter.DEAD_OBJECTS_DELETED))
                        .isGreaterThan(getValue(demoterStatsBefore, JMXCachingConstants.WarmupDemoter.DEAD_OBJECTS_DELETED));
                assertThat(getValue(demoterStatsAfter, JMXCachingConstants.WarmupDemoter.DELETED_BY_LOW_PRIORITY))
                        .isEqualTo(getValue(demoterStatsBefore, JMXCachingConstants.WarmupDemoter.DELETED_BY_LOW_PRIORITY));
            }
            else {
                assertThat(getValue(demoterStatsAfter, JMXCachingConstants.WarmupDemoter.DEAD_OBJECTS_DELETED))
                        .isEqualTo(getValue(demoterStatsBefore, JMXCachingConstants.WarmupDemoter.DEAD_OBJECTS_DELETED));
                assertThat(getValue(demoterStatsAfter, JMXCachingConstants.WarmupDemoter.DELETED_BY_LOW_PRIORITY))
                        .isGreaterThan(getValue(demoterStatsBefore, JMXCachingConstants.WarmupDemoter.DELETED_BY_LOW_PRIORITY));
            }
