...
@TestsEnvironment
public class EnvMultinodeDatabricksUnityS3CredentialsVending
        extends EnvironmentProvider
{
    ...
    @Inject
    public EnvMultinodeDatabricksUnityS3CredentialsVending(StandardMultinode standardMultinode, DockerFiles dockerFiles)
    {
        super(standardMultinode);
        requireNonNull(dockerFiles, "dockerFiles is null");
        configDir = dockerFiles.getDockerFilesHostDirectory("conf/environment/multinode-databricks-unity-s3-credentials-vending");
    }
    ...
    public void extendEnvironment(Environment.Builder builder)
    {
        ...
        builder.configureContainers(container -> {
            if (isTrinoContainer(container.getLogicalName())) {
                container.withEnv("AWS_REGION", awsRegion)
        ...
        builder.configureContainer(TESTS, container ->
                container.withEnv("DATABRICKS_JDBC_URL", databricksTestJdbcUrl)
