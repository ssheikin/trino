...
import io.trino.tests.product.launcher.env.environment.EnvMultinodeWarpGlueHiveCache;
...
public class SuiteWarpSpeedCache
        extends Suite
{
    @Override
    public List<SuiteTestRun> getTestRuns(EnvironmentConfig config)
    {
        return ImmutableList.of(
                testOnEnvironment(EnvMultinodeWarpGlueHiveCache.class)
                        .withGroups(WARP_SPEED_CACHE)
                        .build(),
                testOnEnvironment(EnvMultinodeWarpHiveCache.class)
                        .withGroups(WARP_SPEED_HIVE_CACHE)
                        .build());
    }
}
