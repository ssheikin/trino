        ...
        extends AbstractTestQueryFramework
{
            ...
            throws Exception
    {
        ...
        QueryEvents queryEvents = runQueryAndWaitForEvents(
                ...
                ALTER CATALOG %s SET PROPERTIES
                   ...
                   "connection-password" = '4321'
                """.formatted(catalog));

        assertRedactedQuery("ALTER CATALOG %s SET PROPERTIES \"connection-user\" = 'john', \"connection-password\" = '***'".formatted(catalog), queryEvents);
    }

    @Test
    void testAlterCatalogWithNonExistentCatalog()
            throws Exception
    {
        QueryEvents queryEvents = runQueryAndWaitForEvents(
                """
                ALTER CATALOG nonexistent_catalog SET PROPERTIES
                   "connection-user" = 'john',
                   "connection-password" = '4321'
                """, "Catalog 'nonexistent_catalog' does not exist");

        assertRedactedQuery("ALTER CATALOG nonexistent_catalog SET PROPERTIES \"connection-user\" = '***', \"connection-password\" = '***'", queryEvents);
    }

    @Test
    void testExplainAlterCatalog()
            throws Exception
    {
        String catalog = "catalog_" + randomNameSuffix();
        String connectionUrl = createH2ConnectionUrl();
        ...
        QueryEvents queryEvents = runQueryAndWaitForEvents(
                """
                EXPLAIN ALTER CATALOG %s SET PROPERTIES
                   "connection-user" = 'john',
                   "connection-password" = '4321'
                """.formatted(catalog));
        ...
        assertRedactedQuery(
                """
                EXPLAIN\s
                ALTER CATALOG %s SET PROPERTIES "connection-user" = 'john', "connection-password" = '***'""".formatted(catalog), queryEvents);
    }

    @Test
    void testAlterCatalogWithFunctionCall()
            throws Exception
    {
        String catalog = "catalog_" + randomNameSuffix();
        String connectionUrl = createH2ConnectionUrl();
        ...
        QueryEvents queryEvents = runQueryAndWaitForEvents(
                """
                ALTER CATALOG %s SET PROPERTIES
                   "connection-user" = lower('JOHN'),
                   "connection-password" = lower('SECRET')
                """.formatted(catalog));

        assertRedactedQuery("ALTER CATALOG %s SET PROPERTIES \"connection-user\" = lower('JOHN'), \"connection-password\" = '***'".formatted(catalog), queryEvents);
    }

    @Test
    void testAlterCatalogWithDuplicatedProperties()
            throws Exception
    {
        String catalog = "catalog_" + randomNameSuffix();
        String connectionUrl = createH2ConnectionUrl();
        ...
        QueryEvents queryEvents = runQueryAndWaitForEvents(
                ...
                ALTER CATALOG %s SET PROPERTIES
                   "connection-user" = 'john',
                   "connection-password" = 'secret1',
                   "connection-password" = 'secret2'
                """.formatted(catalog));

        assertRedactedQuery(
                "ALTER CATALOG %s SET PROPERTIES \"connection-user\" = 'john', \"connection-password\" = '***', \"connection-password\" = '***'".formatted(catalog),
                queryEvents);
    }

    @Test
    void testAlterCatalogWithInvalidSecretReference()
            throws Exception
    {
        String catalog = "catalog_" + randomNameSuffix();
        String connectionUrl = createH2ConnectionUrl();
        ...
        QueryEvents queryEvents = runQueryAndWaitForEvents(
                ...
                ALTER CATALOG %s SET PROPERTIES
                   "connection-user" = 'john',
                   "connection-password" = '${ENV:nonexistent}'
                """.formatted(catalog), "Environment variable is not set: nonexistent");

        assertRedactedQuery("ALTER CATALOG %s SET PROPERTIES \"connection-user\" = '***', \"connection-password\" = '***'".formatted(catalog), queryEvents);
    }

    @Test
    void testExecuteImmediateAlterCatalog()
            throws Exception
    {
        String catalog = "catalog_" + randomNameSuffix();
        String connectionUrl = createH2ConnectionUrl();
    ...
    @Test
    void testPrepareAlterCatalog()
            throws Exception
    {
        String catalog = "catalog_" + randomNameSuffix();
        QueryEvents queryEvents = runQueryAndWaitForEvents(
                """
                PREPARE alter_catalog FROM
                ALTER CATALOG %s SET PROPERTIES
                   "connection-user" = ?,
                   "connection-password" = '123'
                """.formatted(catalog));

        // All properties are redacted because we cannot evaluate 'connection-user'.
        assertRedactedQuery(
                """
                PREPARE alter_catalog FROM
                ALTER CATALOG %s SET PROPERTIES "connection-user" = '***', "connection-password" = '***'""".formatted(catalog), queryEvents);
    }

    @Test
    void testExecutePreparedAlterCatalog()
            throws Exception
    {
        String catalog = "catalog_" + randomNameSuffix();
        String connectionUrl = createH2ConnectionUrl();
        ...
        String alterCatalog =
                ...
                """.formatted(catalog);
        Session session = Session.builder(getSession())
                .addPreparedStatement("alter_catalog", alterCatalog)
                .build();

        QueryEvents queryEvents =  queries.runQueryAndWaitForEvents("EXECUTE alter_catalog", session).getQueryEvents();

        String redactedCreateCatalog = "ALTER CATALOG %s SET PROPERTIES \"connection-user\" = 'bob', \"connection-password\" = '***'".formatted(catalog);
        QueryCreatedEvent queryCreatedEvent = queryEvents.getQueryCreatedEvent();
        assertThat(queryCreatedEvent.getMetadata().getQuery())
                .isEqualTo("EXECUTE alter_catalog");
        assertThat(queryCreatedEvent.getMetadata().getPreparedQuery())
                .isEqualTo(Optional.of(redactedCreateCatalog));
    ...
    @Test
    void testExecutePreparedAlterCatalogWithParameters()
            throws Exception
    {
        String catalog = "catalog_" + randomNameSuffix();
        String alterCatalog =
        ...
        Session session = Session.builder(getSession())
                .addPreparedStatement("alter_catalog", alterCatalog)
                .build();
        QueryEvents executeEvents = runQueryAndWaitForEvents(
                "EXECUTE alter_catalog USING 'password'",
                session,
                ".*Incorrect number of parameters: expected 0 but found 1");

        // TODO: Currently, dynamic parameters within the SET PROPERTIES clause are not supported, as demonstrated by the query above.
        //       However, the user will only learn about this limitation when they issue the EXECUTE statement.
        //       We should either prevent the use of dynamic parameters during ALTER CATALOG execution
        //       or redact sensitive properties in the EXECUTE statement.
        assertRedactedQuery("EXECUTE alter_catalog USING 'password'", executeEvents);
    }
