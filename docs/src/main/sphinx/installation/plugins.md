...
many plugins as part of the binary package. Every Trino release publishes each
plugin as a ZIP archive to [GitHub](https://github.com/trinodb/trino/releases).
Refer to [](plugins-list) for details. 

The specific name is derived from the Maven coordinates of each plugin as
...
The coordinates translate into a path to the ZIP archive on GitHub releases. Use
this URL to download the plugin.

For example, the PostgreSQL connector plugin can be downloaded at

```
https://github.com/trinodb/trino/releases/download/|trino_version|/trino-postgresql-|trino_version|.zip
```
